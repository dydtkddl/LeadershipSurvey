...
import { JMupperLayer,result_group_labels } from 'src/components/result_group_labels';
...
export default function ResultJMLeadershipSurvey({ surveyname }) {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("http://127.0.0.1:8000/result_data_render/", {
          ...
          body: JSON.stringify({ surveyname_: surveyname, group_info_ : group_info[JMupperLayer[currentShow]] })
        });
        if (response.ok) {
  ...
  <Grid container spacing={3}>
      {isloading ? <CircularProgress/> : 
        ...
        <Stack width = "100%" direction = "row"  display = "flex"justifyContent="space-around">
      {isloading ? <CircularProgress/> : 
          <GuideCard 
            title={["Personal attributes","and growth mindset"]}
            total={responde.total.toFixed(1)}
            onClick = {()=>(setCurrentShow(0))
            }
            isSelected = {currentShow ===0}
          />}
      {isloading ? <CircularProgress/> : 
          <GuideCard
            ...
            isSelected = {currentShow ===1}

          />}
      {isloading ? <CircularProgress/> : 
          <GuideCard
            title={["Ability to grasp, ","judge, and solve problems"]}
            total={responde.total.toFixed(1)}
            onClick = {()=>(setCurrentShow(2))}
            isSelected = {currentShow ===2}
